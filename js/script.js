...
const DarkMode = {
  ...
  enableDarkMode() {
    ...
    this.updateMetaThemeColor('#0a0e27');
  },
  ...
  enableLightMode() {
    ...
    this.updateMetaThemeColor('#0f1538');
  },
...
const Newsletter = {
  init() {
    ...
    this.btn = document.getElementById('newsletterBtn');
    this.success = document.getElementById('newsletterSuccess');
  ...
  handleSubmit(e) {
    ...
    setTimeout(() => {
      ...
      // Salvar no localStorage para lembrar inscrição
  ...
  setLoading(loading) {
    if (loading) {
      this.btn.classList.add('subscribing');
      this.btn.innerHTML = '<i class="fas fa-spinner"></i> Subscribing...';
    } else {
      this.btn.classList.remove('subscribing');
      this.btn.innerHTML = '<i class="fas fa-paper-plane"></i> Subscribe';
    }
  },
