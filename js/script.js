...
function handleNewsletterSubmit(event) {
  ...
  setTimeout(() => {
    ...
    showToast('Welcome to Procurement, Data & Operations!');
...
/* ============================================
   ENHANCED STAT COUNTER FOR HERO SECTION
   ============================================ */
    ...
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        if (numMatch) {
          ...
          animateHeroCounter(el, targetValue, hasEuro, hasPlus, hasM);
        }
...
function animateHeroCounter(element, target, hasEuro, hasPlus, hasM) {
  const duration = 2000;
  const steps = 60;
  const stepTime = duration / steps;
  let current = 0;
  ...
  const timer = setInterval(() => {
    ...
    let formatted = current.toFixed(target < 10 ? 1 : 0);
    if (hasM) formatted = current.toFixed(0);
