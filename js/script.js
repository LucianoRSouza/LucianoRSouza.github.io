...
// Guardar posição ao abrir galeria de trade shows
const originalOpenTradeGallery = window.openTradeGallery;
window.openTradeGallery = function(brand) {
  tradeShowsScrollY = window.scrollY || document.documentElement.scrollTop || 0;
  if (originalOpenTradeGallery) {
    originalOpenTradeGallery(brand);
  } else {
    // Implementação padrão se não existir
...
/* -------------------------
   Correção 4: Strategy Modal com tradução completa
--------------------------*/
