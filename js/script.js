...
function setupCardAutoSlide(card) { 
 ...
 const interval = Math.max(1200, parseInt(card.getAttribute('data-interval'), 10) 
 || 2500); 
 ...
 const clickable = card.querySelector('.gallery-overlay') 
 || container; 
...
function fixGadsdenImages() { 
 ...
 if (img.src.includes('Gadsden') 
 || img.alt.includes('Gadsden')) { 
...
function initI18N() { 
 ...
 || (['en', 'pt', 'es', 'fr'].includes(browser) ? browser : 'en'); 
...
function ensurePGCloseButton(){ 
 ...
 btn.textContent = (document.documentElement.lang
 ||'pt').startsWith('pt')?'Fechar e Voltar':'Close'; 
...
if(__origOpenPG){ 
 ...
 __pg_lastScrollY = window.scrollY 
 || document.documentElement.scrollTop 
 || 0; 
 __origOpenPG(card); 
 ensurePGCloseButton(); 
...
document.addEventListener('DOMContentLoaded',function(){try{var l=localStorage.getItem('lang')
 ||document.documentElement.lang
 ||'en';if(typeof translateAll==='function'){translateAll(l);}}catch(e){}}); 
...
function openTradeGallery(brand){__pg_lastY=window.scrollY
 ||document.documentElement.scrollTop
 ||0;const images=(TRADE_GALLERIES[brand]
 ||[]).slice();if(!images.length)return;buildProjectSlides(images);document.getElementById('projectGalleryModal').classList.add('active');document.body.style.overflow='hidden';} 
...
(function(){ 
 const getLang = ()=> (window.PG_state && PG_state.currentLang) 
 || (document.documentElement.lang
 ||'en').slice(0,2); 
 const overlay = ()=> document.getElementById('statModalOverlay'); 
 window.openStatModal = function(key){ 
 const lang = getLang(); 
 const dict = (STAT_I18N[lang] 
 ||STAT_I18N.en 
 ||{}); 
 const data = dict[key]; 
 if(!data){ 
 ...
 if(!ov
 ||!data) return; 
 document.getElementById('statModalIcon').className = 'fas ' + (statDetailsData[key]?.icon
 ||'fa-chart-line'); 
 document.getElementById('statModalTitle').textContent = data.title
 ||''; 
 document.getElementById('statModalValue').textContent = data.value
...
// ===== Preserve scroll wrappers ... (restante igual ao seu arquivo) 
// === SAFE LOADER FALLBACK (non-intrusive, does NOT modify your JS logic) ===
(function() {
  ...
  // 1) Se a página já estiver carregada ou quase, esconde rápido
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    setTimeout(hide, 1200);
  } else {
    // 2) Fallback por DOMContentLoaded — cobre 99% dos casos
    document.addEventListener('DOMContentLoaded', function() {
      setTimeout(hide, 1500);
    });
  }

  // 3) Kill-switch absoluto — mesmo se tudo falhar, some em até 5s
