/* =========================================================
   Portfolio JS — consolidado e limpo
   ========================================================= */
// Helpers e estado
...
// Dados — detalhes dos KPIs & Estratégia (mesmo conteúdo do original)
...
function openStrategyModal(num){
  ...
  const body = data.sections.map(sec=>`<div class="strategy-detail-section"><h4><i class="fas fa-chevron-right"></i> ${sec.title}</h4><ul>${sec.items.map(i=>`<li>${i}</li>`).join('')}</ul></div>`).join('');
...
function buildProjectSlides(images){
  const slider = $('#gallerySlider'); const dotsContainer = $('#galleryDots'); if(!slider||!dotsContainer) return;
  ...
  images.forEach((src,idx)=>{
    ...
    const img = document.createElement('img'); img.alt = 'Imagem '+(idx+1); img.src=src; slide.appendChild(img); slider.appendChild(slide);
...
// ---------- UX ----------
function initMobileEnhancements(){ const isTouch = window.matchMedia('(pointer: coarse)').matches; if(!isTouch) return; $$('.stat-box, .strategy-item, .project-card, .gallery-item').forEach(el=>{ on(el,'touchstart',function(){ this.style.transform='scale(0.98)'; },{passive:true}); on(el,'touchend',function(){ this.style.transform=''; },{passive:true}); }); }
function enhanceProjectGalleries(){ const map={
  'blaupunkt-tools': [ './Blaupunkt_Tools.png','./Blaupunkt_Illumiation_booth_HK_Fair.png','./Blaupunkt_Illumiation_booth_HK_Fair_1.png','./Blaupunkt_Illumiation_booth_HK_Fair_2.png','./Blaupunkt_Illumiation_booth_HK_Fair_3.png','./Blaupunkt_Illumiation_booth_HK_Fair_4.png' ],
  'blaupunkt-power': [ './Blaupunkt_Power_Tools.png','./Blaupunkt_Tools.png','./Blaupunkt_Illumiation_booth_HK_Fair.png','./Blaupunkt_Illumiation_booth_HK_Fair_1.png','./Blaupunkt_Illumiation_booth_HK_Fair_2.png' ],
  'blaupunkt-garden':[ './Blaupunkt_Garden_Tools.png','./Blaupunkt_Tools.png','./Blaupunkt_Illumiation_booth_HK_Fair_2.png','./Blaupunkt_Illumiation_booth_HK_Fair_3.png','./Blaupunkt_Illumiation_booth_HK_Fair_4.png' ] };
...
function openLightbox(el){ const lb=$('#lightbox'); const lbImg=$('#lightbox-img'); if(!lb||!lbImg) return; const img = el?.querySelector?.('img'); if(!img?.src) return; lbImg.src = img.src; lb.classList.add('active'); document.body.style.overflow='hidden'; }
...
function initLangSwitcher(){ const switcher=$('#langSwitcher'); if(!switcher) return; on(switcher,'click',(e)=>{ const btn=e.target.closest('.lang-btn'); if(!btn) return; const lang=btn.dataset.lang; if(!lang) return; translateAll(lang); markActiveLang(lang); try{ localStorage.setItem('lang', lang); }catch(e){} showToast(`Traduzido para ${lang.toUpperCase()}`); }); }
function initI18N(){ try{ const stored=localStorage.getItem('lang'); const browser=(navigator.language || 'pt').slice(0,2).toLowerCase(); const initial = stored || (['en','pt','es','fr'].includes(browser) ? browser : 'en'); translateAll(initial); markActiveLang(initial); }catch(e){ translateAll('pt'); markActiveLang('pt'); } }

// Loading & Âncoras
function initLoading(){ const loading=$('#loading'); if(!loading) return; window.addEventListener('load', ()=>{ setTimeout(()=>{ loading.classList.add('hidden'); setTimeout(()=> loading.remove(), 400); }, 800); }); }
...
addEventListener('DOMContentLoaded', ()=>{
  ...
  // Stat boxes
  $$('.stat-box').forEach(box=>{ box.style.cursor='pointer'; on(box,'click',()=>{ const k=box.dataset.stat; if(k && statDetailsData[k]) openStatModal(k); }); });
  console.log('✅ Portfolio JS inicializado');
});

// Export para handlers inline
